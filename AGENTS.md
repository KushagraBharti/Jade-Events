...
- `npm run lint` runs eslint across the repo; fix warnings before opening a PR.
- `npm run build` produces the production bundle; `npm start` serves the built output (use to smoke-test prod config).
...
## Testing Guidelines
- No automated tests are checked in yet. When adding features, include lightweight coverage (React Testing Library for components, Playwright for flows) and run `npm run lint` + `npm run build` to catch regressions.
...
- PRs: include a summary of changes, any linked issue, before/after screenshots for UI updates, and the commands you ran (`npm run lint`, `npm run build`). Call out content or asset placeholders that still need replacement.
